...
if ($nv_Request->isset_request('confirm', 'post')) {
    ...
    if ($checkss == md5($id . NV_CHECK_SESSION)) {
        ...
        nv_insert_logs(NV_LANG_DATA, $module_name, 'Delete Order', 'ID: ' . $id . ' - ' . $row['order_code'], $admin_info['userid']);

        nv_jsonOutput([
            'status' => 'OK',
            'message' => $lang_module['success_delete'],
            'redirect' => NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=orders'
        ]);
    }
}
...
nv_jsonOutput([
    'status' => 'confirm',
    'message' => $lang_module['confirm_delete'] . '<br><strong>' . $row['order_code'] . ' - ' . $row['customer_name'] . '</strong>',
